...
\begin{definition}[Closure property of \REG]
...
The Definition of \textbf{clean grammar} is shown in \ref{def:clean-grammar}.
...
\begin{enumerate}[label=step \arabic*., ref=(step \arabic*), labelindent=1em, leftmargin=*, widest=step 2.]
  \item compute the \textit{set} \(\text{def} \subseteq V\) of the well defined \textbf{non-terminals}
        \begin{itemize}
          \item \text{def} is initialized with the non-terminals that occur in the terminal rules \textit{(the rules having a terminal as their \RP)}
          ...
          \item this transformation is applied repeated until convergence is reached
                ...
                \begin{itemize}
                  \item each symbol \(D_i, 1 \leq i \leq n\) is either a terminal in \(\Sigma\) or a non-terminal in \(\text{def}\) already
...
\bigskip
The definition introduces a sufficient and a necessary condition for the language to be infinite.
...
The definition of left and right derivation is shown in Definition~\ref{def:left-right-derivation}.
...
\bigskip
At any time, the future behaviour of the machine depends on the \textbf{instantaneous configuration}, defined in Definition~\ref{def:instantaneous-configuration}.
...
\begin{property}[Equivalente Clean Automaton]
...
The distinguishability relation can be computed through its inductive Definition \textit{(\ref{def:distinguishability-inductive})}.
...
The formal definition of Nondeterministic Finite Automaton is shown in Definition~\ref{def:nondeterministic-finite-automaton}.
...
For a language \(L\) over an alphabet \(\Sigma\), the local sets are called \Ini, \Fin, and \Dig, and are defined in \textit{(\ref{def:set-of-initials}, \ref{def:set-of-finals}, \ref{def:set-of-digrams})}.
...
\bigskip
Finally, it's possible to define the \textbf{Local Language} \textit{(Definition~\ref{def:local-language})}.
...
A formal definition of a Pushdown Automaton is shown in~\textit{(\ref{def:pushdown-automaton})}.
...
\begin{definition}[Deterministic Pushdown Automata and Deterministic Language]
...
\bigskip
A formal definition of simple deterministic grammar is shown in Definition~\ref{def:simple-deterministic-grammar}.
...
\bigskip
A formal definition is given in \textit{(\ref{def:grammar-as-network-of-automata})}.
...
If \(M_A\) is the axiom machine, \(q_A\) is the final state and \(\texttt{cc} = \dashv\), then accept and \texttt{halt}.
...
\begin{property}[Unambiguity conditions for translations]
  ...
  The the the translation specified by grammar \(G_\tau\) is single valued and defines a translation function.
\end{property}
...
The subjacent automaton of the translator is obtained by erasing the the target alphabet symbols and the output string actions from the definitions.
...
\begin{enumerate}
  ...
  \item No rule may contain the following \textbf{substrings}:
        \[ \dfrac{\varepsilon}{v_1} \dfrac{a}{v_2} \ \text{or} \ \dfrac{\varepsilon}{v_1} \dfrac{\varepsilon}{v_2} \]
        ...
        \begin{itemize}
          \item[\(\rightarrow\)] if such combinations are present in a rule, they can be respectively \textbf{replaced} by the equivalent pairs
            \[ \dfrac{a}{v_1 v_2} \ \text{or} \ \dfrac{\varepsilon}{v_1 v_2} \]
        \end{itemize}
\end{enumerate}
...
The rules for constructing the transducer moves are in Table~\ref{tab:pushwdown-transducer-construction-algorithm}.
...
The Nivat theorem \textit{(shown in Theorem~\ref{theo:nivat-theorem})} states some properties of the transition relation.
...
Sequential Transducers are automaton used to efficiently compute the translation in real time, while reading the input tape.
...
and means that in the state \(q\), while reading character \(a\), emit string \(u\) and move to the next state \(r\).
...
\[ \left\{ yz \in \Delta^\ast \mid \exists \ \text{a computation labelled} \ \dfrac{x}{y} \ \text{ending in} \ r \in F \land z = \phi\left( r, \dashv \right) \right\} \]
...
However, in such cases the translation can be implemented via two cascaded \textit{(deterministic)} sequential passes, each one being one way but scanning the string in opposite directions:

\begin{enumerate}[label=step \arabic*., ref=(step \arabic*), leftmargin=*, widest=step 2.]
  \item a sequential transducer scans from left to right and converts the source string into an intermediate string
  \item another sequential transducer scans the intermediate string from right to left and produces the specified target string
\end{enumerate}
...
\begin{enumerate}[label=\arabic*., ref=(\arabic*)]
  \item\label{enum:syntax-directed-compiler-phase-1} Parsing or syntax analysis
  \item\label{enum:syntax-directed-compiler-phase-2} Semantic evaluation or semantic analysis
\end{enumerate}
...
The semantic phase \textit{(Phase~\ref{enum:syntax-directed-compiler-phase-2})} consists in applying a set of \textbf{semantic functions} on each node of the tree until all attributes have been evaluated;
...
The The syntax-directed translator contains the the definition of the semantic functions, which are associated with the grammar rules.
The se of grammar rules and associated semantic functions is called an \textbf{attribute grammar}.
...
\bigskip
A formal definition of attribute grammar is shown in Definition~\ref{def:attribute-grammar}.
...
\begin{definition}[Attribute grammar]
  ...
  \begin{enumerate}
    ...
    \item A \textbf{set of symbols}, the \textit{(semantic)} attributes, associated with nonterminal and terminal syntax symbols.
          The set of the attributes associated with symbol \(\square\) is denoted \(\textit{attr}\left( \square \right)\).
    ...
    \item A \textbf{set of semantic functions} \textit{(or rules)}.
          \begin{itemize}
            ...
            \item a \textbf{semantic function} has the \textbf{form}
                  \[ \sigma_k \coloneqq f \left( \textit{attr} \left( \left\{ D_0, D_1, \ldots, D_k \right\} \right) \setminus \left\{ \sigma_k \right\}\right) \quad 0 \leq k \leq r \]
                  function \(f\) assigns to the attribute \(\sigma\) of symbol \(D_k\) the value computed by the function body and its arguments can be any attributes of teh same production \(p\), excluding \(\sigma_k\) itself.
          ...
          \end{itemize}
    \item The set \(\textit{fun}(p)\) of functions supported by production \(p\) must satisfy the following \textbf{conditions}:
          \begin{enumerate}
            \item for each left attribute \(\sigma_0\) of \(D_0\), the exists in \(\textit{fun}(p)\) \textbf{exactly one function} defining the attribute
            \item For each right attribute \(\delta_0\) of \(D_0\), \textbf{no function} exists in \(\textit{fun}(p)\) defining the attribute
            \item For each left attribute \(\sigma_i, \ i \geq 1\), \textbf{no function }exists in \(\textit{fun}(p)\) defining the attribute
            \item For each attribute \(\delta_i, \ i \geq 1\), there exists in \(\textit{fun}(p)\) \textbf{exactly one function} defining the attribute
          \end{enumerate}
          \begin{itemize}
        ...
        \begin{itemize}
          \item before entering and evaluating a subtree \(t_N\), it computes the right attributes of node \(N\) \textit{(the root of the subtree)}
                \begin{itemize}[label=\(\rightarrow\)]
                  ...
                  \item procedure calls with input parameters passing passing are the \inlinequote{descending phase} of the visit
                \end{itemize}
          \item at the end of visit of subtree \(t_N\), it computes the left attributes of node \(N\)
                \begin{itemize}[label=\(\rightarrow\)]
                  ...
                  \item procedure return and output parameter passing are the \inlinequote{ascending phase} of the visit
                \end{itemize}
        \end{itemize}
\end{enumerate}
...
The \textbf{lexical analyser} \textit{(or scanner)} segments the source text into the lexical elements, called \textbf{lexemes} \textit{(or tokes)}, such as identifiers, integers and strings;
...
Assume that a syntax is suitable for deterministic top-down parsing \textit{(as shown in Section~\ref{sec:top-down})}; attribute evaluation can proceed with parsing if the functional dependencies of the grammar obey additional conditions beyond the one-sweep grammar conditions:
...
\begin{definition}[L-condition]
  ...
  \begin{enumerate}[label=\alph*., ref=(\alph*)]
    \item\label{enum:l-condition-1} The one-sweep condition \textit{(Definition~\ref{def:one-sweep-grammar-condition}, page~\pageref{def:one-sweep-grammar-condition})} is satisfied
    \item\label{enum:l-condition-2} The sibling graph \(\textit{sibl}_p\) \textit{(introduced in Paragraph~\ref{par:one-sweep-grammar}, page~\pageref{par:one-sweep-grammar})} contains arc \(D_j \rightarrow D_i, \ j > i \geq 1\)
  \end{enumerate}
\end{definition}
...
\begin{definition}[Attribute grammar and deterministic parsing]
  ...
  It's possible to construct a top-down deterministic parser with attribute evaluation able to compute the attributes at parsing time.
\end{definition}
...
\begin{itemize}
  \item verification, to examine the program correctness
  \item optimization, to improve the efficiency of the program
  \item scheduling and parallelizing, to change the execution order of the program in order to exploit the parallelism
\end{itemize}
...
The equations are defined in Definition~\ref{def:data-flow-equations}.
...
A formal definition of the Reaching Definition problem is given in Definition~\ref{def:reaching-definition}.
...
A formal definition of badly initialized variables is given in Definition~\ref{def:badly-initialized-variables}.
