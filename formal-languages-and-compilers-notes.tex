...
The reflection and star properties of \CF have already been proven in the previous Sections, while: % TODO add references
...
such list is called the \textbf{Chomsky hierarchy} and is shown in Table~\ref{tab:chomsky-hierarchy}.
...
\[ L(\alpha) = \left\{ x \in \Sigma^\ast \, | \, \alpha(x) = \texttt{yes} \right\}\]
...
This move is also sometimes denoted as \(q \xrightarrow{a} r\).
...
\[ L(M) = \left\{ x \in \Sigma^\ast \, | \, \delta(q_0, x) \in F \right\} \]
...
\subsubsection{Clearing algorithm}
...
\begin{itemize}
  \item alternate moves - grammar with two alternatives \(A \rightarrow aB \, | \, aC, a \in \Sigma\)
  \item spontaneous moves - grammar with copy rule \(A \rightarrow \varepsilon\)
\end{itemize}
...
\[ L(N) = \left\{ x \in \Sigma^\ast \, | \, q \xrightarrow{x} r \text{ with } q \in I \land r \in F \right\} \]
...
\begin{align*}
  ...
  \forall \, q \in Q, \, \forall \, y \in \Sigma^\ast \quad & \delta(q, y) = \left[ p \, | \, q \xrightarrow{y} p \right]
\end{align*}
...
\[ L(N) = \left\{ x \in \Sigma^\ast \, | \, \exists \, q \in I \text{ such that } \delta(q, x) \cap F \neq \emptyset \right\} \]
...
The pair of alternatives \(p \rightarrow a q \, | \, ar\) corresponds to a pair of nondeterministic moves \textit{(rule \(3\))}.
...
\begin{enumerate}
  ...
  \item replacement of multiple nondeterminism transitions with one transition that enters a new state
        \begin{itemize}[label=\(\rightarrow\)]
          \item this phase is called powerset constructions, as the new states constitute a subset of the state set
...
\begin{lstlisting}[language=pseudocode, caption={Direct elimination of spontaneous moves}, label={lst:direct-elimination-of-spontaneous-moves}]
...
\[ L \setminus \left\{ \varepsilon \right\} = \left\{ x \, | \, \Ini(x) \in \Ini(L) \land \Fin(x) \in \Fin(L) \land \Dig(x) \subseteq \Dig(L) \right\} \]
...
\begin{enumerate}
  \item the initial character is in \(\Ini(L)\)
  \item any pairs of adjacent characters are in \(\Dig(L)\)
  \item the final character is in \(\Fin(L)\)
\end{enumerate}
...
\[ \Fol(a) = \left\{ b \, | \, ab \in \Dig(e^\prime \dashv) \right\} \]
...
\begin{lstlisting}[language=pseudocode, caption={Berry and Sethi Algorithm}, label=lst:berry-sethi-algorithm]
...
The \textit{BS} algorithm is a valid alternative to the powerset construction (seen in ) % TODO add reference
...
As already seen, the \REG family is closed under complementation and intersection. % TODO add reference
Let \(L\) and \(L^\prime\) be two regular languages. Their complement \(\lnot L\) and intersection \(L \cap L^\prime\) are regular languages as well.
...
it's created exploiting the De Morgan identity \(L_1 \cap L_2 = \lnot (\lnot L_1 \cup \lnot L_2)\).
Therefore the algorithm is:

\begin{enumerate}
  \item Build the deterministic recognizers of \(L_1\) and \(L_2\)
  \item Derive the recognizers of \(\lnot L_1\) and \(\lnot L_2\)
  \item Build the recognizers of \(\lnot L_1 \cup \lnot L_2\) by using the Thompson method % TODO add reference
